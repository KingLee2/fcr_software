...
void uart_write(){
    ...
    if(data_tran_local.size()==num_byte_pc_stm){
        ...
        view_data("data_send_uart: ",data_tran_local);
    }
    //
    try{
...
void uart_read(){
    ...
    cout<<"Size data receive uart: "<<data_receive_local.size()<<endl;
    ...
    if(send_command_shutdown!=0) {
        ...
        sleep(2);
        if(send_command_shutdown==1) system("sudo systemctl poweroff -i");
        //system("shutdown -h now");
        else if(send_command_shutdown==2) system("sudo systemctl reboot -i");
        //system("reboot");
        exit(0);
    }
    // check frame data
    ...
    if(data_error==0){
        ...
        button=data_receive_uart[button1_re];
        if(button==0){
